resource "azurerm_virtual_machine_scale_set" "user25-vmss" {
name = "user25-vmss"
...
network_profile {
        ...
        load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.user25-bep.id]
        load_balancer_inbound_nat_rules_ids = [azurerm_lb_nat_pool.lb-natpool.id]
    }
